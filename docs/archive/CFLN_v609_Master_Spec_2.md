...
class CFLNModel(nn.Module):
    ...
    def _update_telescoping(self,x_c_final_chunk: torch.Tensor,s_t: float=0.0) -> torch.Tensor:
        ...
        L_compress=((chunk_mean-x_recon_1).conj()*(chunk_mean-x_recon_1)).real.sum()  # v6.0.9 fix: removed .detach() so CFL5Layer receives gradient to produce compressible representations
...
def train_step_v605(batch, model, opts, si, phase, step,
    ...
    L_compress_w=cfg.get('lambda_compress',0.001)  # reduced from 0.01: upstream gradient now flows through L_compress
